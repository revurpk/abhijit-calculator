...
**Solar-noon time slider.** The moment picker carries a slider whose midpoint is that day's **solar noon**; dragging left/right sets the time as `solarNoon + offset` (±720 min, clamped within the day) and re-renders every card live (`skySliderInput`). The thumb and readout ("Nh Mm before/after solar noon") are kept in sync by `syncSkySlider(solarNoon, localMin)` whenever the time changes by any means — typing, "Now", or selecting a search result — so the slider always reflects the chosen moment. Useful for nudging the Lagna past a tyajya zone or watching a varga flip around midday.

**Overlay auto-compute.** Each participant's birth fields (date, time, city or lat/lon) carry an `onchange` that calls `autoSlotStar(i)` — as soon as enough is entered (date + time + a city or lat+lon) the birth moment is computed and overlaid on the charts automatically, without needing the explicit "→ star" button. Picking a bare nakshatra from the dropdown still scores the participant's Tarabala/Chandrabala but produces no chart overlay (there is no birth moment to plot).
